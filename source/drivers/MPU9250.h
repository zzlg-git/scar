...
extern void MPU9250_Write_Reg(uint8 register_address, uint8 data);//SPI写
extern uint8 MPU9250_Read_Reg(uint8 register_address);//SPI读
extern void MPU9250_ReadBytes(uint8 REG_Address,uint8 length,uint8 *data);
extern void MPU9250_writeBit(uint8 reg, uint8 bitNum, uint8 data);
extern void Mag_write(uint8 reg,uint8 value);
extern uint8 Mag_read(uint8 reg);
extern void MPU9250_Init_spisoft(void);
extern int16 MPU9250_GetResult_spisoft(uint8 register_address);

extern void readdmp(void);
extern long getdmplong(unsigned char address);
extern void getquaternion(void);
extern void getyawpitchroll(void);
extern void getAcc_gyro();
extern void getmag();
extern void loadfirmware(void);
extern void loadcfgupd(void);
extern void MPU9250_Sleep();
extern void MPU9250_WakeUp();
...
uint8 IICreadBytes(uint8 SlaveAddress,uint8 REG_Address,uint8 len,uint8 *data);
uint8 IICwriteBytes(uint8 dev, uint8 reg, uint8 length, uint8* data);
uint8 IICreadByte(uint8 dev, uint8 reg, uint8 *data);
uint8 IICwriteByte(uint8 dev, uint8 reg, uint8 data);
