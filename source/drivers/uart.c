...
#error "Definition UART_CHANNEL ERROR ! "
    ...
    int uart_baud = UART_BAUD ;/// 3;//test
    sbr = (int)(((int64)periph_clk_khz*1000) / (uart_baud*16));
    brfa = (int)(((int64)periph_clk_khz*1000*32) / (uart_baud*16) - sbr * 32);
    if(ABS((int64)periph_clk_khz*1000*10 / (32*sbr+brfa+1) - uart_baud*5)
       < ABS((int64)periph_clk_khz*1000*10 / (32*sbr+brfa) - uart_baud*5))
        brfa = brfa + 1;
    ...
    UART_C2_REG(UART_CHAN_PTR) |= UART_C2_TE_MASK | UART_C2_RE_MASK|UART_C2_RIE_MASK;
}
